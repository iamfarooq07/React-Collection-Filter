    ...
    return (
      <div className="min-h-screen py-12">
        <div className="container mx-auto px-4 text-center">
          ...
          <Link
            to="/products"
            className="inline-block bg-gray-500 text-white px-8 py-3 rounded-lg font-semibold hover:bg-gray-700 transition-colors"
          >
            Browse Products
          </Link>
        </div>
        ...
        <div className="grid grid-cols-12 gap-8">
          <div className="col-span-8 bg-white text-black rounded-lg shadow-md">
            ...
            <div>
              {cartItems.map((item) => (
                <CartItem
                  key={`${item.id}-${item.cartDrink?.id ?? "noDrink"}`}
                  item={item}
                />
              ))}
            </div>
